  ...
  <aside class="w-80 bg-[#0f172a]/40 backdrop-blur-2xl border-r border-white/5 flex flex-col shadow-[20px_0_50px_rgba(0,0,0,0.5)] z-20">
    <div class="p-8">
      ...
      <div class="space-y-10">
        <div>
          ...
          <input v-model="startUrl" class="w-full bg-slate-900/60 border border-white/10 rounded-2xl px-5 py-4 text-sm focus:border-indigo-500 transition-all outline-none" />
        </div>
        ...
        <div>
          ...
          <div class="flex bg-slate-900/80 p-1.5 rounded-2xl border border-white/5">
            <button v-for="d in [1,2,3]" :key="d" @click="depth = d" 
              :class="depth === d ? 'bg-indigo-600 text-white' : 'text-slate-500'"
        ...
        <div>
          ...
          <div class="mt-2 space-y-2">
            <input v-model="auth.username" type="text" placeholder="Username" class="w-full p-2 text-xs bg-slate-800/50 border border-slate-700 rounded text-white outline-none" />
            <input v-model="auth.password" type="password" placeholder="Password" class="w-full p-2 text-xs bg-slate-800/50 border border-slate-700 rounded text-white outline-none" />
          </div>
        </div>

        <button @click="handleEmit" :disabled="loading" class="w-full relative group overflow-hidden bg-white text-black font-black py-5 rounded-2xl transition-all">
          ...
          <span class="relative z-10 group-hover:text-white transition-colors tracking-widest uppercase text-xs">
            {{ loading ? 'Mapping Architecture...' : 'Generate Neural Map' }}
          </span>
        </button>
      </div>
    </div>
    ...
    <div class="mt-auto p-8 border-t border-white/5 bg-gradient-to-t from-indigo-500/5 to-transparent">
      <div class="flex items-center justify-between">
        ...
        <div class="w-2 h-2 rounded-full bg-emerald-500" :class="{'animate-pulse': loading}"></div>
      </div>
    </div>
  </aside>
</template> -->
